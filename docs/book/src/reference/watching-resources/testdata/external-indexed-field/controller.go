...
func (r *ConfigDeploymentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	*/
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &appsv1.ConfigDeployment{}, configMapField, func(rawObj runtime.Object) []string {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			&handler.EnqueueRequestsFromMapFunc{
				ToRequests: handler.ToRequestsFunc(r.findObjectsForConfigMap),
			},
...
*/
func (r *ConfigDeploymentReconciler) findObjectsForConfigMap(configMap handler.MapObject) []reconcile.Request {
	attachedConfigDeployments := &appsv1.ConfigDeploymentList{}
	listOps := &client.ListOptions{
		FieldSelector: fields.OneTermEqualSelector(configMapField, configMap.Meta.GetName()),
